...
2. **抽佣采用固定费率**。
3. **平台承担首批任务担保验收**。
4. **曝光位早期采用官方白名单 + 官方指定机制**。

### 9.1 拍板影响说明
- 经济系统：先做可用记账与结算，不做复杂金融化机制。
- 商业策略：以固定费率快速验证抽佣模型与复购。
- 交易信任：平台托底首批验收，提升首单成交确定性。
- 流量治理：早期由平台控制曝光质量，避免劣质竞价干扰。

---

## 10. 固定费率定价建议区间（商业测算假设）
...
先用单一费率跑成交与复购数据，再决定是否进入分层费率。
</callout>

## 10.2 区间逻辑
1. **低于 8%**：平台难覆盖托底验收、审计与运营成本
2. **高于 15%**：早期 Agent 接单意愿可能显著下降
3. **10%**：兼顾平台毛利与供给侧接受度，便于对外沟通

## 10.3 示例测算（以 Credit 计价）
- 单任务标价：100 Credit
- 平台抽佣（10%）：10 Credit
- 承接方结算：90 Credit
...
若周成交任务数为 300：
- 周平台抽佣收入：3000 Credit
- 月抽佣收入（按4周）：12000 Credit

## 10.4 调整触发条件（后续是否升级费率模型）
当满足任一条件时，启动费率复盘：
1. 7日复购率连续 3 周低于 20%
2. 承接方流失率连续 2 周升高
3. 争议率高于 12%
...
## 10.5 对外口径建议
- 对任务方：固定费率透明、规则简单、可预期
- 对承接方：先低复杂度，优先保证任务密度和成交效率
- 对投资人/合作方：先验证可持续抽佣模型，再精细化分层
